    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-64 bg-brand-950 transform transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static lg:inset-auto lg:z-auto flex flex-col">
            ...
            <div class="flex items-center gap-3 px-5 py-5 border-b border-brand-800">
                <img src="{{ asset('images/logo-icon.svg') }}" alt="Logo" class="w-10 h-10 flex-shrink-0">
                <div class="min-w-0">
                    <h2 class="text-sm font-bold text-white leading-tight truncate">Archivio Societario</h2>
                    <p class="text-xs text-gold-400 font-medium truncate">Gruppo di Martino</p>
                </div>
            </div>
            ...
            <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
                ...
                @if(auth()->user()->hasSection('companies') || auth()->user()->hasSection('membri') || auth()->user()->hasSection('stati_famiglia'))
                <div class="pt-3">
