    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-64 bg-brand-950 transform transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static lg:inset-auto lg:z-auto flex flex-col">
            ...
            <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
                ...
                @if(auth()->user()->hasSection('companies') || auth()->user()->hasSection('membri') || auth()->user()->hasSection('stati_famiglia') || auth()->user()->hasSection('libri_sociali') || auth()->user()->isAdmin())
                <div class="pt-3">
                ...
                @if(auth()->user()->hasSection('libri_sociali'))
                ...
                <a href="{{ route('registri-contabili.index') }}" class="{{ request()->routeIs('registri-contabili.*') ? 'sidebar-link-active' : 'sidebar-link' }}">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 11h6m-6 4h6M5 3h14a2 2 0 012 2v14a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2z"/></svg>
                    <span>Libri e Registri Contabili</span>
                </a>
                @endif
